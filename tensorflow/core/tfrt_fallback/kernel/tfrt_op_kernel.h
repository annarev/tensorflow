...
#ifndef TENSORFLOW_CORE_TFRT_FALLBACK_KERNEL_TFRT_OP_KERNEL_H_
#define TENSORFLOW_CORE_TFRT_FALLBACK_KERNEL_TFRT_OP_KERNEL_H_
...
//#include
//"tfrt/backends/common/include/tfrt/common/compat/eigen/thread_pool_device.h"
...
// tfrt::KernelFrame. Outputs and errors are stored internally.
class TFRTOpKernelContext {
 ...
 private:
  ...
  // KernelFrame because we must temporarily store allocate_output's Tensor
  // somewhere until the Tensor is initialized. If we stored the allocated
  // Tensor directly in the KernelFrame, the frame's output becomes available
  // and downstream kernels may use the allocated (but uninitialized) Tensor.
...
#endif  // TENSORFLOW_CORE_TFRT_FALLBACK_KERNEL_TFRT_OP_KERNEL_H_
